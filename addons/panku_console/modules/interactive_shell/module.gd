...
func get_intro() -> String:
	...
	intro.append("[font_size=56][color=#ffffff44][b][i]> Moose Console[/i][/b][/color][/font_size]")
	intro.append("[font_size=18][color=#ffffff44]Version: %s([url=%s][color=#10a00c]%s[/color][/url])" % [PankuUtils.get_plugin_version(), PankuUtils.get_commit_url(), PankuUtils.get_commit_sha_short()])
	return "\n".join(intro)
...
func init_module():
	...
	window.set_window_title_text("Moose Shell V2")
